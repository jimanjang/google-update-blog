...
const WEBHOOKS = {
  ADMIN_ROOM: {
    ...
    url: 'https://chat.googleapis.com/v1/spaces/AAAAET8D3kw/messages?key=AIzaSyDdI0hCZtE6vySjMm-WEfRq3CPzqKqqsHI&token=sMLNYUUIr-kf8OOileiorbFW3Gi0WpTawy8Fm0nXv3Q'
  },
};
...
// 번역 함수 추가
function translateText_(text, targetLang = 'ko') {
  try {
    let translatedText = LanguageApp.translate(text, 'en', targetLang);
...
function buildUpdateObject_(feed, id, date, title, content, link) {
  ...
  // 번역 추가
  let translatedTitle = translateText_(title);
  let translatedSnippet = translateText_(snippet);
  let translatedFullContent = translateText_(fullContent);
  ...
  return {
    ...
    fullContent: (translatedFullContent.length > 0) ? '...' + translatedFullContent : '',
    link: link
  };
}
